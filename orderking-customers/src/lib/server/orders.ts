...
export const placeOrder = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input: { restaurantId: string; zoneId: string; lat: number; lng: number; coupon?: string | null; tipPaise?: number; lines: CartLineInput[]; address: { line1: string; area: string; landmark?: string; instructions?: string; label?: string }; paymentMethod: "COD" | "UPI_SANDBOX" | "KING_PAY"; notes?: string; idempotencyKey: string }) => input).handler(async ({ context, data }) => {
  if (!rateLimit(`order:${context.userId}`, 5, 60_000)) throw new Error("Too many order attempts. Wait a minute."); if (!data.address.line1.trim()) throw new Error("Delivery address is required."); if (data.lines.length === 0) throw new Error("Cart is empty."); const sql = await getSql(); const existing = await sql<{ id: string; public_id: string }>`select id, public_id from orders where idempotency_key = ${data.idempotencyKey} and user_id = ${context.userId}`; if (existing[0]) return { orderId: existing[0].id, publicId: existing[0].public_id, duplicate: true }; const first = await isFirstOrder(context.userId); const built = await buildQuote({ restaurantId: data.restaurantId, zoneId: data.zoneId, lat: data.lat, lng: data.lng, coupon: data.coupon, lines: data.lines }, first); if (built.result.quote.blockers.length) throw new Error(`Order blocked: ${built.result.quote.blockers.join(", ")}`); const cfg = await loadConfig(); if (cfg.marketplace.launchMode === "live") {
      ...
      return { orderId: created.data.orderId, publicId: created.data.publicId, duplicate: false, deliveryOtp: "1234" }; // HDmaster handles OTP, but we mock 1234 here or fetch it
