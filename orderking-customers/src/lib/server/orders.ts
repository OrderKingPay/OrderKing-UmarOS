...
function localSimulationEnabled(): boolean {
  return (
    process.env.ORDERKING_ALLOW_LOCAL_SIMULATION?.trim().toLowerCase() === "true" &&
    process.env.VERCEL_ENV !== "production"
  );
}

function requireLiveOrExplicitSimulation(launchMode: string): void {
  if (launchMode === "live" || localSimulationEnabled()) return;
  throw new Error("Order data is unavailable until HDmaster live mode is connected.");
}
