...
export function parseFounderQuery(query: string, founderUpiVpa: string = "orderking@okhdfcbank"): {
  ...
  if (isHindi) detectedLanguage = "hi-IN";
    ...
    if (q.startsWith("/ops") || q.startsWith("/zomato") || q.includes("process payout") || q.includes("generate payout") || q.includes("resolve dispute") || q.includes("auto operations")) {
    return founderBlockedResponse("autonomous_ops", detectedLanguage, "Live payout/dispute operations are not connected to a canonical persisted operations service; no mutation is executed.");
  }

  if (q.startsWith("/pay") || q.startsWith("/ledger") || q.startsWith("/kingpay") || q.includes("verify ledger") || q.includes("process payment") || q.includes("run settlement")) {
    return founderBlockedResponse("kingpay_ledger", detectedLanguage, "Live payment/ledger settlement is not connected to a canonical transaction service; no money movement is executed.");
  }
