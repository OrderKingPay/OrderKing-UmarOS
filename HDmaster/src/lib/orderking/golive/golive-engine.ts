...
export const DEFAULT_GOLIVE_CONFIG: MasterGoLiveConfig = {
  ...
  paymentGateway: {
    provider: "RAZORPAY",
    mode: (process.env.PAYMENT_MODE === "PRODUCTION" ? "PRODUCTION" : "SANDBOX") as "SANDBOX" | "PRODUCTION",
    apiKey: process.env.PAYMENT_API_KEY ?? "",
    secretKey: process.env.PAYMENT_API_SECRET ?? "",
  ...
  smsGateway: {
    provider: (process.env.SMS_PROVIDER ?? "FAST2SMS") as SmsGatewayConfig["provider"],
    apiKey: process.env.SMS_API_KEY ?? "",
    senderId: process.env.SMS_SENDER_ID ?? "",
    dltEntityId: process.env.SMS_DLT_ENTITY_ID ?? "",
    dltTemplateIdOtp: process.env.SMS_DLT_TEMPLATE_ID_OTP ?? "",
    dltTemplateIdOrder: process.env.SMS_DLT_TEMPLATE_ID_ORDER ?? "",
    otpLength: 4,
  ...
  maps: {
    provider: (process.env.MAPS_PROVIDER ?? "MAPBOX") as MapsConfig["provider"],
  ...
  legal: {
    ...
    registrationNumberCin: process.env.LEGAL_CIN ?? "",
    panNumber: process.env.LEGAL_PAN ?? "",
    tanNumber: process.env.LEGAL_TAN ?? "",
    registeredOfficeAddress: process.env.LEGAL_REGISTERED_ADDRESS ?? "",
    stateJurisdiction: "Assam (18)",
    bankAccountHolderName: process.env.LEGAL_BANK_ACCOUNT_HOLDER ?? "",
    bankName: process.env.LEGAL_BANK_NAME ?? "",
    bankAccountNumber: process.env.LEGAL_BANK_ACCOUNT_NUMBER ?? "",
    bankIfscCode: process.env.LEGAL_BANK_IFSC ?? "",
    nodalEscrowId: process.env.LEGAL_NODAL_ESCROW_ID ?? "",
    upiVpaHandle: process.env.LEGAL_UPI_VPA ?? "",
    gstinNumber: process.env.LEGAL_GSTIN ?? "",
    foodGstPercentage: 5,
    platformFeeGstPercentage: 18,
    tcsSection52Enabled: true,
    fssaiLicenseNumber: process.env.LEGAL_FSSAI ?? "",
    fssaiExpiryDate: process.env.LEGAL_FSSAI_EXPIRY ?? "",
