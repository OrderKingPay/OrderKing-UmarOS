...
async function run() {
  ...
  let res = await sql.query(`SELECT state FROM autonomous_tasks WHERE id = $1`, [taskId]);
  assert.equal(res.rows[0].state, 'REQUESTED');
  
  await updateTaskState(taskId, 'EXECUTING', { progress: 0.5 });
  res = await sql.query(`SELECT state, progress FROM autonomous_tasks WHERE id = $1`, [taskId]);
  assert.equal(res.rows[0].state, 'EXECUTING');
  assert.equal(res.rows[0].progress, 0.5);

  await updateTaskState(taskId, 'COMPLETED', { result: { success: true }, evidence: 'Report saved to disk.' });
  res = await sql.query(`SELECT state, result_json, evidence_text FROM autonomous_tasks WHERE id = $1`, [taskId]);
  assert.equal(res.rows[0].state, 'COMPLETED');
  assert.equal(JSON.parse(res.rows[0].result_json).success, true);
  assert.equal(res.rows[0].evidence_text, 'Report saved to disk.');
